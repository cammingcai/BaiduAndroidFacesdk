...
public class FaceLivenessActivity extends Activity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TAG,"mDisplayWidth111onCreate="+mDisplayWidth+"===mDisplayHeight111="+mDisplayHeight);
...
//        if(this.getResources().getConfiguration().orientation!= Configuration.ORIENTATION_LANDSCAPE)//不是横屏
//        {
...
//        }
        ...
        Log.i(TAG,"mPreviewDegree="+mPreviewDegree);
        ...
        Log.i(TAG,"mPreviewHight="+mPreviewHight+",,,mPreviewWidth="+mPreviewWidth);
        Log.i(TAG,"mDisplayWidth="+mDisplayWidth+"===mDisplayHeight="+mDisplayHeight);
        //mPreviewHight=486,,,mPreviewWidth=864    mDisplayWidth=720===mDisplayHeight=1280
        ...
        try {
            ...
            mCamera.setPreviewCallback(this);//调用该接口 就会回调onPreviewFame
...
//            Log.i(TAG, "FaceDetectRoundView detectRect=" +detectRect.toString());
//            Log.i(TAG, "mFaceDetectRoundView.mFaceRect=" +mFaceDetectRoundView.mFaceRect.toString());
//            Log.i(TAG, "FaceDetectRoundView centerX=" +detectRect.centerX()+"--------centerY=" +detectRect.centerY());
//            Log.i(TAG, "mFaceDetectRoundView.mFaceRect=" +mFaceDetectRoundView.mFaceRect.toString());
//            Log.i(TAG, "mFaceRect centerX=" +mFaceDetectRoundView.mFaceRect.centerX()+"--------centerY=" +mFaceDetectRoundView.mFaceRect.centerY());
//            mILivenessStrategy.setLivenessStrategyConfig(
//                    mFaceConfig.getLivenessTypeList(), mPreviewRect, mFaceDetectRoundView.mFaceRect, this);

            //   FaceDetectRoundView detectRect=Rect(0, 194 - 720, 914)
            // FaceDetectRoundView mPreviewRect=Rect(0, 0 - 486, 864)
            Log.i(TAG, "FaceDetectRoundView detectRect=" +mFaceDetectRoundView.getFaceRoundRect().toString());
            Log.i(TAG, "FaceDetectRoundView mPreviewRect=" +mPreviewRect.toString());
            mILivenessStrategy.setLivenessStrategyConfig(
                    mFaceConfig.getLivenessTypeList(), mPreviewRect,mFaceDetectRoundView.getFaceRoundRect(), this);
        }
    ...
    private void onRefreshView(FaceStatusEnum status, String message) {
        ...
        switch (status){
            ...
            case Error_Timeout:
                ...
                startPreview();
                break;
            case OK://最终检测成功，活体检测成功
            //case Liveness_OK://上一个动作检测完成，比如提示眨眨眼  ，用户眨眼了，检测到了，回调这个OK
           // case Liveness_Completion:
                stopPreview();
                startPreview();
                break;
            case Detect_FacePointOut://检测到人脸，但是脸不在框内，位置不好
                break;
            case Detect_PoorIllumintion://光线较暗
                break;
            case Detect_NoFace://没有检测到人脸
                break;
            case Detect_DataHitLast://检测到开始有人脸数据
                break;
            case Detect_PitchOutOfUpMaxRange://头抬得有点高啦
                break;
            case Detect_PitchOutOfDownMaxRange://头有点低啦
                break;
            case Detect_PitchOutOfLeftMaxRange://头有点偏左啦
                break;
            case Detect_PitchOutOfRightMaxRange://头有点偏右啦
                break;
        }
       // Log.i(TAG,);
//        switch (status) {
...
//                onRefreshTipsView(false, message);
//                mTipsBottomView.setText(""+status);
//                mFaceDetectRoundView.processDrawState(false);

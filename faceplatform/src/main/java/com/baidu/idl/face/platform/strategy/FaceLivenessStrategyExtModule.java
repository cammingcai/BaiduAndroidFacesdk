...
//        }
      //  Log.i(TAG,"processStrategy imageData"+imageData);
        FaceModel model = mFaceModule.detect(imageData, mPreviewRect.height(), mPreviewRect.width());
        Log.i(TAG,"processStrategy imageData"+imageData+",--mPreviewRect.height()="
                +mPreviewRect.height()+",--width"+ mPreviewRect.width());
        processUIStrategy(new UILivenessResultRunnable(model));
    }

    private void processUIResult(FaceModel model) {
        //Log.i(TAG,"FaceModel="+model);
        //Log.i(TAG,"mIsProcessing="+mIsProcessing);//true
        if (!mIsProcessing) {
            return;
        }
        ...
        if (model != null
            ...
            //Log.i(TAG,"decodeStatus="+decodeStatus);
            faceInfo = model.getFaceInfos()[0];
            Log.i(TAG,"faceInfo="+faceInfo);
...
//                    break;
    ...
    private boolean processUITips(FaceStatusEnum status) {
       // Log.i("FaceLiveness1122","status="+status);
        boolean flag = false;
        if (status != null) {
    ...
    private void processUICallback(FaceStatusEnum status) {
        Log.i("FaceLiveness11","status="+status);
        Log.i("FaceLiveness11","mLaunchTime="+mLaunchTime);
        Log.i("FaceLiveness11","mNoFaceTime="+mNoFaceTime);
      //  Log.i("FaceLiveness11","mLivenessTipsTime="+mLivenessTipsTime);
      //  Log.i("FaceLiveness11","mLivenessTipsDurationTime="+mLivenessTipsDurationTime);
     //  Log.e(TAG, "switch " + mLivenessStatus.name() + "-" + mLivenessStrategy.getCurrentLivenessStatus());
...
//            Log.e(TAG, "processUICallback " + status.name());
    ...
    private class UILivenessResultRunnable implements Runnable {
        ...
        @Override
        public void run() {
           // Log.i(TAG,"UILivenessResultRunnable");
